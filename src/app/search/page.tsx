...
const Page = async ({ searchParams }: PageProps) => {
  ...
  if (products.length < 3) {
    ...
    const vector = await hfVectorize(query);
  ...
  if (products.length === 0) {
    return (
      <div className=" z-10 mt-4 text-center py-4 bg-background border shadow-md rounded-md">
        <h3 className="mt-2 text-sm font-semibold ">No results</h3>
        <p className="mt-1 text-sm mx-auto max-w-prose text-neutral-400">
  ...
  return (
    <ul className=" z-10 mt-4 py-4 divide-y border divide-border bg-background shadow-md rounded-md">
      {products.slice(0, 3).map((product) => (
        <Link key={product.id} href={`/products/${product.id}`}>
          <li className="mx-auto py-4 px-8 sm:flex space-y-2 sm:space-y-0 sm:space-x-4">
            <div className="relative flex items-center overflow-hidden rounded-lg h-40 w-40">
              <Image
                loading="eager"
                fill
                alt="product-image"
            ...
            <div className="w-full flex-1 space-y-2 py-1">
              ...
              <p className="prose prose-sm text-neutral-400 line-clamp-3">
                {product.description}
              </p>
